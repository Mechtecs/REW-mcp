...
describe('executeApiGenerator', () => {
  ...
  describe('status action', () => {
    ...
    it('should handle null/undefined status values gracefully', async () => {
      ...
      expect(result.status).toBe('success');
      expect(result.data?.generator_status?.playing).toBe(false);
      expect(result.data?.generator_status?.signal).toBe('unknown');
    });

    it('should use signal from composed getGeneratorStatus', async () => {
      mockClient.getGeneratorStatus.mockResolvedValue({
        signal: 'Pink noise',
        level: -18,
        level_unit: 'dBFS',
    ...
    it('should use level from composed getGeneratorStatus', async () => {
      mockClient.getGeneratorStatus.mockResolvedValue({
        signal: 'Pink noise',
        level: -15,
        level_unit: 'dBFS',
