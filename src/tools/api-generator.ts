...
export async function executeApiGenerator(input: ApiGeneratorInput): Promise<ToolResponse<ApiGeneratorResult>> {
  try {
    ...
    switch (validated.action) {
      case 'status': {
        const [composedStatus, frequency] = await Promise.all([
        ...
        return {
          ...
          data: {
            action: 'status',
            success: true,
            message: `Generator signal: ${composedStatus.signal}`,
            generator_status: {
              enabled: true,
              playing: composedStatus.available_commands?.includes('Stop') ?? false,
              signal: composedStatus.signal,
              level_db: composedStatus.level,
              frequency_hz: frequency ?? undefined
            },
            available_commands: composedStatus.available_commands
          }
        };
      }
