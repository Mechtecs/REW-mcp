...
export const ApiMeasureInputSchema = z.object({
  action: z.enum(['status', 'sweep', 'spl', 'cancel', 'configure', 'protection', 'playback_mode', 'measurement_mode'])
  ...
  mode: z.string().optional()
    .describe('Mode value (for playback_mode or measurement_mode actions)'),

  protection_options: z.unknown().optional()
    .describe('Protection options (for protection action with set)')
});
...
export async function executeApiMeasure(input: ApiMeasureInput): Promise<ToolResponse<ApiMeasureResult>> {
  try {
    ...
    switch (validated.action) {
      ...
      case 'protection': {
        if (validated.protection_options !== undefined) {
          const success = await client.setMeasureProtectionOptions(validated.protection_options);
          ...
          return {
            ...
            data: {
              action: 'protection',
              success,
              message: success ? 'Protection options updated' : 'Failed to update protection options'
            }
          };
        }

        const protectionOptions = await client.getMeasureProtectionOptions();
        ...
        return {
          ...
          data: {
            action: 'protection',
            success: true,
            message: 'Protection options retrieved',
      ...
      case 'playback_mode': {
        if (validated.mode) {
          const success = await client.setMeasurePlaybackMode(validated.mode);
          ...
          return {
            ...
            data: {
              ...
              success,
              message: success
                ? `Playback mode set to: ${validated.mode}`
                : 'Failed to set playback mode'
            }
          };
        }

        const mode = await client.getMeasurePlaybackMode();
        ...
        return {
          ...
          data: {
            action: 'playback_mode',
            success: true,
            message: `Current playback mode: ${mode}`,
            playback_mode: mode
          }
        };
      }
      ...
      case 'measurement_mode': {
        if (validated.mode) {
          const success = await client.setMeasurementMode(validated.mode);

          return {
            status: 'success',
            data: {
              action: 'measurement_mode',
              success,
              message: success
                ? `Measurement mode set to: ${validated.mode}`
                : 'Failed to set measurement mode'
            }
          };
        }

        const mode = await client.getMeasurementMode();
        ...
        return {
          ...
          data: {
            action: 'measurement_mode',
            success: true,
            message: `Current measurement mode: ${mode}`,
      ...
      default:
        return {
          ...
          suggestion: 'Use one of: status, sweep, spl, cancel, configure, protection, playback_mode, measurement_mode'
        };
    }
