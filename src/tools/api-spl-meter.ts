...
export const ApiSPLMeterInputSchema = z.object({
  action: z.enum(['start', 'stop', 'read', 'configure', 'list_meters'])
    .describe('SPL meter action to perform'),

  meter_id: z.number().int().min(1).default(1)
    .describe('SPL meter ID (starts at 1)'),
    ...
    switch (validated.action) {
      case 'list_meters': {
        ...
        return {
          ...
          data: {
            action: 'list_meters',
            success: true,
            message: `${Array.isArray(meters) ? meters.length : 0} SPL meters available`,
            meter_id: meterId,
      ...
      default:
        return {
          ...
          suggestion: 'Use one of: list_meters, start, stop, read, configure'
        };
    }
